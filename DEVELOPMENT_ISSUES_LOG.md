...
**实现方案：**
- 修改HTML结构，在`.profile-avatar`中添加单个头像和多个字符头像元素
- 重构CSS样式，分别定义单个头像和多个字符头像的样式
- 通过CSS选择器控制不同主题模式下的显示状态
...
<div class="profile-avatar">
    ...
    <!-- 四个字符头像（深色模式显示） -->
    <div class="multi-avatars">
        <div class="char-avatar char-1">A</div>
        <div class="char-avatar char-2">B</div>
        <div class="char-avatar char-3">C</div>
        <div class="char-avatar char-4">D</div>
    </div>
</div>
```

2. **CSS样式定义：**
- 单个头像：保持原有SVG背景图和渐变效果
- 四个字符头像：使用不同颜色的渐变背景，定位在容器四角
- 主题切换：通过`body:not(.dark-mode)`和`body.dark-mode`选择器控制显示

3. **视觉效果：**
- 浅色模式：显示单个SVG头像图标，具有品牌色边框
- 深色模式：显示四个彩色字符头像（A、B、C、D），分布在四个角落
- 渐变背景：每个字符头像使用不同的品牌色渐变
